<?php if(!defined('BASEPATH')) exit('No direct script access allowed');
    ...
    {        
        ...
        foreach($attendance_of_students as $row) 
        {
            ...
            // commented by diptesh, enabling this this give 500 is attendance update status is 2
...
//Created by sumit
    function issue_request() 
    {
        ...
        $data['page_name']  = 'issue_request';
        $data['page_title'] = get_phrase('issue_request');
        $this->load->view('backend/index', $data);
    }
    ...
    function set_student()
    {
        ...
        $data['studentdetails']= $this->db->get()->result();
        $this->load->view('backend/admin/studentdetails',$data);
    }
    function get_student()
    {
        ...
        $this->load->view('backend/admin/details',$data);
    }
     function set_barcode()
     {
        ...
        $data['barcodedetails']= $this->db->get()->result();
        $this->load->view('backend/admin/barcodedetails',$data);
     }
    ...
    function return_request() 
    {
        ...
        $data['page_name']  = 'return_request';
        $data['page_title'] = get_phrase('return_request');
        $this->load->view('backend/index', $data);
    ...
    function book_issue()
    {
       if($this->input->post())
       {
        //check for session->not done
        $barcode = $this->input->post('barcode');
        $student_id = $this->input->post('studentid');
        $issuedate = date('Y-m-d', strtotime($this->input->post('issuedate')));
        $returndate =  date('Y-m-d', strtotime($this->input->post('returndate')));
        $book_id  = $this->db->get_where('book', array('barcode' => $barcode))->row()->book_id;

        //extra check if value empty->done
        //check if book_id is unique or not

        $this->db->select('*');
        $this->db->from('book_request');
        $this->db->where('book_id',$book_id);
        $this->db->where('status',1);
        $studentexits = $this->db->get()->result();

        if($studentexits)
        {
            $responses = array( 
                         "status"=>0, 
                         "message"=>"Same Book already issued");
            echo json_encode($responses);
        }else
        {
               $inputArray =  array('student_id' => $student_id,
                             'book_id' => $book_id,
                             'issue_start_date'=> date('d-m-Y', strtotime($issuedate)),
                             'issue_end_date'=>date('d-m-Y',strtotime($returndate)),
                             'status' => 1);
               $isInsert = $this->db->insert('book_request', $inputArray);
               if($isInsert)
               {
                $responses = array( 
                         "status"=>1, 
	...
	function retrn_book()
    {
       if($this->input->post())
       {
        ...
        }else
        {
			...
			$this->db->where('student_id', $student_id);
			$this->db->where('book_id', $book_id); 			   
            $return_book = $this->db->update('book_request', $inputArray);
            if($return_book)
            {
            $responses = array( 
                "status"=>3, 
    ...
    function fees()
    {
       if ($this->session->userdata('admin_login') != 1)
        ...
        $data['page_name']  = 'fees';
        $data['page_title'] = get_phrase('fees');
        $this->load->view('backend/index', $data);
    }
	function add_fees_structure()
    {
       if ($this->session->userdata('admin_login') != 1)
        {
            redirect(base_url(), 'refresh');
        }
        $data['page_name']  = 'add_fees_structure';
        $data['page_title'] = get_phrase('add_fees_structure');
        $this->load->view('backend/index', $data);
    }
    function add_fee()
    {
        ...
        $data['page_name']  = 'addfee';
        $data['page_title'] = get_phrase('addfees');
        $this->load->view('backend/index', $data);       
    }

	function add_fees_insert()
    {
       if($this->input->post())
       {
        //check for session->not done
        ...
        $lastdate =  date('Y-m-d', strtotime($this->input->post('last_submsn_date')));
        //echo("inside add fees insert@@@");
        //extra check if value empty->done
        //check if book_id is unique or not
        /*$this->db->select('*');
        ...
        $strctureCheck = $this->db->get()->result();
        ...
        if($strctureCheck) 
        {
            $responses = array( 
                         "class_id"=>$classid,
						 "month_id"=>$month_id,
                         "message"=>"Same structure already exist");
            echo json_encode($responses);
        }else
        {*/
               $inputArray =  array('class_id' => $classid,
                             'month_id' => $month_id,
							 'type_id' => $type_id,
							 'amount' => $amount,
                             ...
                             'fine_id' => 1);
                // echo(inputArray);
               $isInsert = $this->db->insert('fee_structure', $inputArray);
               if($isInsert)
               {
                $responses = array( 
                        "class_id"=>$classid,
    ...
    function add_concession_data()
    {
       if($this->input->post())
       {
        ...
        $amount = $this->input->post('amount');
               ...
               $inputArray1 =  array('class_id' => $class_id,
                             'section' => $section,
							 'student' => $student,
                ...
                // echo(inputArray1);
                // print_r(inputArray1);
               $isInsert1 = $this->db->insert('concessions', $inputArray1);
               if($isInsert1)
               {
                $responses = array( 
                        "class_id"=>$class_id,
    ...
    function add_discount_data()
    {
       if($this->input->post())
       {
        ...
        $amount = $this->input->post('amount');
               ...
               $inputArray2 =  array('class_id' => $class_id,
                             'section' => $section,
							 'student' => $student,
							 'running_year' => $running_year,
                             'month' => $month,
                ...
                // echo(inputArray1);
                // print_r(inputArray1);
               $isInsert2 = $this->db->insert('discounts', $inputArray2);
               if($isInsert2)
               {
                $responses = array( 
                        "class_id"=>$class_id,
	...
	function get_feesStructure($class_id = '')
    {        
        $sms_status = $this->db->get_where('settings' , array('type' => 'sms_status'))->row()->description;
        $notify = $this->db->get_where('settings' , array('type' => 'absences'))->row()->description;
        $running_year = $this->db->get_where('settings' , array('type' => 'running_year'))->row()->description;
	...
	function fees_discount()
    {
        ...
        $data['page_name']  = 'fees_discount';
        $data['page_title'] = get_phrase('fees_discount');
        $this->load->view('backend/index', $data);       
    }

    function fees_concession()
    {
        //check admin...
        if ($this->session->userdata('admin_login') != 1)
        {
        ...
        $data['page_name']  = 'fees_concession';
        $data['page_title'] = get_phrase('fees_concession');
        $this->load->view('backend/index', $data);       
    }
