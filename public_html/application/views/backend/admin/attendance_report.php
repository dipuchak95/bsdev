                      ...
                      <td nowrap> <img alt="" src="<?php echo $this->crud_model->get_image_url('student',$row['student_id']);?>" width="20px" style="border-radius:20px;margin-right:5px;"> <?php echo $this->crud_model->get_name('student', $row['student_id']);?> </td>
                        <?php
                                ...
                                {   
                                    ...
                                    $timestamps = ($year . '-' . $month . '-' . $i);
                                    
                                    $day = date('Y-m-d',strtotime($timestamps));
                                    $this->db->group_by('timestamp');
                                    // echo "SectionID: ".$section_id." ClassID: ".$class_id." RunningYear: ".$running_year." Timestamp: ".$day." Date: ".$day." StudentID: ".$row['student_id']."<br>";
                                    $attendance = $this->db->get_where('attendance', array('section_id' => $section_id, 'class_id' => $class_id, 'year' => $running_year, 'timestamp' => $day, 'student_id' => $row['student_id']))->result_array();
                                    // echo(count($attendance));

                                    foreach ($attendance as $row2):
                                      // echo $row2['timestamp'];
                                    // echo "Attendance status: ".$row2['status']."<br>";
                                    $month_dummy = date('d', strtotime($row2['timestamp']));
                                    // echo "Month Dummy Value is: ".$month_dummy;
