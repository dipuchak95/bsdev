	...
	<div class="os-tabs-w menu-shad">
		<div class="os-tabs-controls">
			<ul class="navs navs-tabs">
				<li class="navs-item">
				  <a class="navs-links <?php if($page_name == 'general_reports') echo "active";?>" href="<?php echo base_url();?>admin/general_reports/"><i class="picons-thin-icon-thin-0658_cup_place_winner_award_prize_achievement"></i> <span><?php echo get_phrase('classes');?></span></a>
				</li>
				<li class="navs-item">
					...
					<?php if($class_id != "" && $section_id != ""):?>
						<div class="row">
							<div class="text-center col-sm-6"><br>
								...
								<p><b><?php echo get_phrase('teacher');?>:</b> <?php echo $this->crud_model->get_name('teacher', $this->db->get_where('class', array('class_id' => $class_id))->row()->teacher_id);?><br><b><?php $this->db->where('class_id', $class_id); $this->db->where('section_id', $section_id); echo $this->db->count_all_results('enroll');?></b> <?php echo get_phrase('students');?>  |  <b><?php $this->db->where('class_id', $class_id); echo $this->db->count_all_results('subject');?></b> <?php echo get_phrase('subjects');?>.<br><b><?php echo get_phrase('running_year');?>:</b> <?php echo $this->db->get_where('settings', array('type' => 'running_year'))->row()->description;?></p>
							</div>
							<div class="col-sm-6 text-center">
								<div class="up-main-info">
    								<div class="user-avatar-w"> 
          							...
          							<h4 class="up-header"><?php echo $this->crud_model->get_name('admin', $this->db->get_where('class', array('class_id' => $class_id))->row()->teacher_id);?></h4>
      									...
      									<table width="100%" class="table table-lightborder table-lightfont">
      										...
      										<tbody>
        										...
        										<tr>
          											...
          											<td style="text-align: center;"><a class="btn btn-rounded btn-sm btn-purple" style="color:white"><?php echo $this->crud_model->get_name('teacher', $subject['subject_id']);?></a></td>
        										</tr>
      											<?php endforeach;?>
      										</tbody>
      									</table>
